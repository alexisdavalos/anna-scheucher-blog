	...
	useEffect(() => {
		if (typeof window !== "undefined" && data) {
			...
			async function updateViewCount() {
				...
				const res = await axiosInstance.post(`/api/posts/update/${post.slug}`, {
					data: { ...data },
				});
				...
				{router.isFallback ? (
					...
					<>
						<article className="pb-16 mt-8 md:mt-16 border-b-4">
							...
							<p className="text-xl mt-24 mb-3 hidden md:block ">Written By:</p>
							<Avatar
...
export async function getStaticProps({ params }) {
	...
	try {
		...
		if (res && res.data.data === null) {
			...
			const res = await axiosInstance.post(`/api/posts/update/${params.slug}`, {
				data,
			});
			if (res && res.data.data) {
				data = res.data.data;
